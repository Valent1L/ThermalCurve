...
class ComparisonPlot:
    ...
    def draw_statistics(
        ...
        for item in statistics:
            ...
            color = self.colors.setdefault(
                f"group:{group.identifier}:{signal}",
                COMPARISON_COLORS[len(self.colors) % len(COMPARISON_COLORS)],
            )
            if show_individual:
                for identifier in item.included:
                    curve = curves_by_id[identifier]
                    ...
                    if len(options.signals) > 1:
                        label_text += f" — {_t(SIGNAL_NAMES[signal])}"
                    line = axis.plot(
                        x, y + curve.y_offset, color=self.colors.setdefault(
                            curve.identifier,
                            COMPARISON_COLORS[len(self.colors) % len(COMPARISON_COLORS)],
                        ), linewidth=curve.line_width, linestyle=curve.line_style,
            ...
            if show_mean:
                ...
                if len(options.signals) > 1:
                    label_text += f" — {_t(SIGNAL_NAMES[signal])}"
                line = axis.plot(
                    item.x, item.mean, color=color, linewidth=2.0, label=label_text
                )[0]
                lines.append(line)
                labels.append(label_text)
            ...
            if show_band:
                ...
                if finite.any():
                    axis.fill_between(
                        item.x, item.mean - item.std, item.mean + item.std,
    ...
    def draw_zones(self, curves, options: ComparisonOptions, *, show_heat_surfaces: bool = True) -> None:
        ...
        for zone in options.plot_options.analysis_zones:
            ...
            self.axis.axvspan(zone.start, zone.end, color="#6A5ACD", alpha=0.20 if active else 0.08)
