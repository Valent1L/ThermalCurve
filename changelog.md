...
- Distribution portable Windows x64 : Python officiel 3.14.6, bibliothèques validées et lanceur `Lancer_ThermalCurve.cmd` inclus, sans installation ni `.venv` chez l'utilisateur. Conservation des sources, exemples, icône et licences ; sources tierces proposées séparément. Lanceur isolé des installations Python externes ; aucun `ThermalCurve.exe` PyInstaller. README, description de release et mail adaptés. Calculs scientifiques inchangés.
- Documentation : message Windows Defender raccourci dans les deux README, la description de release et le mail, avec mention d'une possible version exécutable dans une future mise à jour si le blocage est résolu.
...
- Portable Windows x64 distribution: include official Python 3.14.6, validated libraries and `Lancer_ThermalCurve.cmd`, without installation or a user `.venv`. Preserve sources, examples, icon and licenses; provide third-party sources separately. Launcher isolated from external Python installations; no PyInstaller `ThermalCurve.exe`. Update READMEs, release description and email. Scientific calculations unchanged.
- Documentation: shortened the Windows Defender notice in both READMEs, the release description and the email, mentioning a possible executable version in a future update if the block is resolved.
